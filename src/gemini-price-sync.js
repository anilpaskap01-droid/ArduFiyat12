...
export function syncTargets(db, productIds = []) {
  ...
  return db.offers
    .filter((offer) => {
      ...
      return Boolean(
        refreshable &&
        product?.active &&
        (!selectedProductIds.size || selectedProductIds.has(product.id)) &&
        store?.active &&
...
async function runGeminiJob(job, apiKey, productIds) {
  const client = new GoogleGenAI({ apiKey });
  const targets = syncTargets(readDb(), productIds);
...
export function startGeminiPriceSync(options = {}) {
  ...
  const productIds = Array.isArray(options.productIds)
    ? [...new Set(options.productIds.map((value) => String(value || '').trim()).filter(Boolean))]
    : [];
  ...
  currentJob.selectedProductIds = productIds;

  currentJobPromise = runGeminiJob(currentJob, apiKey, productIds)
    .catch(async (error) => {
