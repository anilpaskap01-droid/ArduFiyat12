const state = {
  ...
  selectedGeminiProductIds: new Set()
};
...
function renderDashboard() {
  ...
  const productsWithOffers = state.data.collections.products.filter((product) =>
    state.data.collections.offers.some((offer) => offer.productId === product.id)
  );
  ...
  $('#adminContent').innerHTML = `
    ...
    <div class="dash-grid">
      ...
      <div class="sync-card">
        ...
        <div class="gemini-product-picker">
          <div class="gemini-picker-head">
            <div><b>Kontrol edilecek ürünler</b><small id="geminiSelectionCount">0 ürün seçildi</small></div>
            <div><button type="button" class="text-btn" id="selectAllGeminiProducts">Tümünü seç</button><button type="button" class="text-btn" id="clearGeminiProducts">Temizle</button></div>
          </div>
          <div class="gemini-product-list">
            ${productsWithOffers.map((product) => `
              <label class="gemini-product-option">
                <input type="checkbox" data-gemini-product="${esc(product.id)}" ${state.selectedGeminiProductIds.has(product.id) ? 'checked' : ''}>
                <img src="${esc(product.imageUrl || `/api/product-image/${encodeURIComponent(product.id)}`)}" alt="" loading="lazy" onerror="this.style.display='none'">
                <span><b>${esc(product.name)}</b><small>${state.data.collections.offers.filter((offer) => offer.productId === product.id).length} teklif</small></span>
              </label>
            `).join('')}
          </div>
        </div>
  ...
  const updateSelection = () => {
    state.selectedGeminiProductIds = new Set(
      [...document.querySelectorAll('[data-gemini-product]:checked')].map((input) => input.dataset.geminiProduct)
    );
    const count = $('#geminiSelectionCount');
    if (count) count.textContent = `${state.selectedGeminiProductIds.size} ürün seçildi`;
  };
  document.querySelectorAll('[data-gemini-product]').forEach((input) => input.addEventListener('change', updateSelection));
  $('#selectAllGeminiProducts').onclick = () => {
    document.querySelectorAll('[data-gemini-product]').forEach((input) => { input.checked = true; });
    updateSelection();
  };
  $('#clearGeminiProducts').onclick = () => {
    document.querySelectorAll('[data-gemini-product]').forEach((input) => { input.checked = false; });
    updateSelection();
  };
  updateSelection();
...
function geminiChanges(job) {
  const changes = Array.isArray(job?.changes) ? job.changes : [];
  if (!changes.length) return '';
  return `<div style="display:grid;gap:5px;margin-top:8px">${changes.map((change) =>
    `<span><b>${esc(change.productName || change.offerId)}</b> · ${esc(change.storeName || '')}: ${money(change.previousPrice)} → ${money(change.nextPrice)}</span>`
  ).join('')}</div>`;
}
...
async function refreshOffersWithGemini() {
  const productIds = [...state.selectedGeminiProductIds];
  if (!productIds.length) {
    toast('Önce fotoğraflı listeden en az bir ürün seçin.');
    return;
  }
  const offerCount = state.data.collections.offers.filter((offer) => productIds.includes(offer.productId)).length;
  if (!confirm(`${productIds.length} ürünün ${offerCount} teklifi Gemini ile kontrol edilecek. Devam edilsin mi?`)) {
    return;
  }
