  ...
  offers: {
    fields: [
      ...
      [
        ...
        'select',
        null,
        ['manual_verified', 'merchant_csv', 'official_api', 'admin_import']
      ],
      ['active', 'Aktif', 'checkbox']
    ]
  },
...
function renderDashboard() {
  ...
  $('#adminContent').innerHTML = `
    ...
    <div class="dash-grid">
      ...
      <div class="sync-card">
        <span class="eyebrow">TEKLİF YÖNETİMİ</span>
        <h3>Teklif listesini güncelle</h3>
        <p>
          Kayıtları yeniden yükler ve varsa CSV/API kaynağındaki yeni teklifleri işler.
          Manuel teklifler doğrulanmadan otomatik olarak değiştirilmez.
        </p>
        <button class="primary-btn" id="dashSync">Teklifleri Yenile</button>
      </div>
    </div>
  `;
...
function renderTable(collection) {
  ...
  const refreshButton = collection === 'offers'
    ? '<button class="secondary-btn compact-btn" id="tableRefreshOffers">Teklifleri Yenile</button>'
    : '';
