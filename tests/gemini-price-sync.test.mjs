...
test('selected products limit Gemini offer targets', () => {
  const db = createDatabase();
  db.products = [
    { id: 'product_uno', name: 'Arduino Uno', sku: 'UNO', active: true },
    { id: 'product_nano', name: 'Arduino Nano', sku: 'NANO', active: true },
    { id: 'product_esp32', name: 'ESP32', sku: 'ESP32', active: true }
  ];
  db.stores[0].domain = 'store.example.com';

  const selectedTargets = syncTargets(db, ['product_nano']);

  assert.equal(selectedTargets.length, 1);
  assert.equal(selectedTargets[0].productId, 'product_nano');
  assert.equal(selectedTargets[0].offerId, 'offer_stock');
});
